...
# dati <- read.csv(file = "dati_modello2.csv")
# ind <- sample(2, nrow(dati), replace = TRUE, prob = c(0.8, 0.2))
# F_train <- dati[ind==1,]
# F_test <- dati[ind==2,]
...
#read.csv("dati_modello2.csv")
data <- h2o.importFile("dati_modello2.csv")  # 163,987 rows x 15 columns
dim(data)
# [1] 163987     15
...
data$label <- as.factor(data$label)  #encode the binary repsonse as a factor
h2o.levels(data$label)  #optional: after encoding, this shows the two factor levels, '0' and '1'
# [1] "0" "1"
...
data[1:20,1]
y <- "label"
x <- setdiff(names(data), c('C1',y))  #remove the interest rate column because it's correlated with the outcome
print(x)
# [1] "loan_amnt"             "term"                 
...
gbm_fit3 <- h2o.gbm(x = x,
                    ...
                    #                    stopping_metric = "AUC",      #used for early stopping ??only for binary response??
...
dl_fit2 <- h2o.deeplearning(x = x,
                            ...
                            epochs = 100,
                            nfolds = 10,
                            hidden= c(1000,1000,1000,1000,1000,1000),
...
summary(dl_fit3)
head(as.data.frame(h2o.varimp(dl_fit3)))
